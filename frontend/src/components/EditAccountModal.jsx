...
export default function EditAccountModal({ account, onClose, onSaved }) {
  ...
  const [colour, setColour] = useState(account.colour || '#6366f1');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-sm bg-slate-900 border border-slate-700 rounded-xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
          <h2 className="text-sm font-semibold text-slate-100">Edit Account</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1.5 font-medium">Tax Wrapper / Subtype</label>
            <select

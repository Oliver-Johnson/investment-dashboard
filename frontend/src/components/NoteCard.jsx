...
function NoteContent({ note, maxH }) {
  return (
...
function NoteFooter({ note, accounts, onEdit, onDelete, confirmDelete, setConfirmDelete }) {
...
export default function NoteCard({ note, accounts, onEdit, onDelete }) {
  ...
  const footerProps = { note, accounts, onEdit, onDelete, confirmDelete, setConfirmDelete };
  ...
  return (
    <>
      <motion.div
        ...
        className="bg-slate-900 border rounded-xl overflow-hidden cursor-pointer"
        style={{ borderColor: colour + '40' }}
        onClick={() => setExpanded(true)}
      >
        <div className="h-0.5 w-full" style={{ backgroundColor: colour }} />
        <NoteContent note={note} maxH="max-h-64" />
        <NoteFooter {...footerProps} />
      </motion.div>
      ...
      {expanded && createPortal(
        ...
        >
          <div
            className="bg-slate-900 border rounded-xl overflow-hidden w-full max-w-2xl shadow-2xl"
            style={{ borderColor: colour + '60' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="h-0.5 w-full" style={{ backgroundColor: colour }} />
            <NoteContent note={note} maxH="max-h-[70vh]" />
            <NoteFooter {...footerProps} />
          </div>
        </div>,
        document.body
